...
public class EditorPredictivo extends ABB<String> {
    ...
    public EditorPredictivo(String nombreFichero) {   
        ...
        try {         
            ...
            fPalabras.close();
            this.raiz = super.construirEquilibrado(palabras, 0, palabras.length-1);
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {
        ...
        int counter = n;        
        if( recuperar(prefijo) != null ){ result.insertar(prefijo); counter--;}
        String sucesor = sucesor(prefijo);
        while(n > 0 && sucesor.startsWith(prefijo) && sucesor != null){
            result.insertar(sucesor);
            n--;

...
public class GrafoMunicipios {
    ...
    public double distancia(Municipio m1, Municipio m2) {
        return grafo.pesoArista(/*COMPLETAR*/ obtenerCodigo(m1), /*COMPLETAR*/ obtenerCodigo(m2));
    }
    ...
    public ListaConPI<Municipio> caminoMinimo(Municipio mOrig, Municipio mDst) {    
        ...
        int origen= obtenerCodigo(mOrig);
        int destino= obtenerCodigo(mDst);
        ListaConPI<Municipio> camino= new LEGListaConPI<Municipio>();
        if(origen!=-1 && destino !=-1){
        ListaConPI<Integer> res= grafo.caminoMinimo(origen,destino);
        for(res.inicio();!res.esFin();res.siguiente()){
            camino.insertar(getMunicipio(res.recuperar()));
          }
        }

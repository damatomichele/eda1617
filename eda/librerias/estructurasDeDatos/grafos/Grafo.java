...
public abstract class Grafo {
    ...
    protected void dijkstra(int origen) {
        ...
        ColaPrioridad<AlcanzableDijkstra> cp = new PriorityQColaPrioridad<>();
        cp.insertar(new AlcanzableDijkstra(origen, 0));
        distanciaMin[origen] = 0;
        
        while(!cp.esVacia()){
            int v= cp.eliminarMin().vertice;
            if (visitados[v]==0){
                visitados[v]=1;
                ...
                for(listaW.inicio();!listaW.esFin();listaW.siguiente()){
                    ...
                    cp.insertar(new AlcanzableDijkstra(w.destino, distanciaMin[w.destino]));
                    }
                }
            }
        }
    ...
    public ListaConPI<Integer> caminoMinimo(int origen, int destino) {      
     ...
     public boolean pasaPor(int o,int d, int i){
        ListaConPI<Integer> aux=caminoMinimo(o,d);
